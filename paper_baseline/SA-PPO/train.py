...
writer = SummaryWriter(log_dir=os.path.join(experiment_dir, "runs_ha_10x5"))
...
for i in range(START_EPISODE, TOTAL_EPISODES + 1):
    ...
    with torch.no_grad():
        while not done:
            ...
            # C. AGV Agent
            agvaction = myenv.get_heuristic_agv_action(jobaction)

            real_agv_action = agvaction
            # E. Env Full Step
    ...
    if i % 10 == 0:
        ...
        writer.add_scalar("Train/Total_Reward", job_score, i)
    ...
    if is_quick_eval or is_full_eval:
        ...
        for case_data in eval_candidates:
            ...
            while not eval_done:
                ...
                a_act = eval_env.get_heuristic_agv_action(j_act)
                real_a = a_act
