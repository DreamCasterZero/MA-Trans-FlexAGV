...
class EnvWorkShop(gym.Env):
    ...
    def get_heuristic_agv_action(self, job_action):
        """
        [HA-PPO 专用] 基于最近距离原则选择 AGV
        """
        real_job_idx = job_action - 1
        job = self.FJSP.Jobs[real_job_idx]
        target_pos = job.cur_pos  # 工件当前位置

        best_agv_id_1based = -1 # 改为存储物理 ID
        min_cost = 999999.0

        num_jobs = len(self.FJSP.Jobs)

        for i, agv in enumerate(self.FJSP.AGVs):
            # 计算成本：AGV 到达工件位置的距离 + AGV 释放的等待时间
            dist = config.agv_trans[agv.cur_pos][target_pos]
            wait_time = max(0, agv.end - self.last_makespan)
            cost = dist + wait_time

            if cost < min_cost:
                min_cost = cost
                best_agv_id_1based = i + 1
        return best_agv_id_1based
