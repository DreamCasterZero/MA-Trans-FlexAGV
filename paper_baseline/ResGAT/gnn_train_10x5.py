...
writer = SummaryWriter(log_dir=os.path.join(experiment_dir, "runs_gnn_10x5"))
...
if START_EPISODE > 0:
    ...
    try:
        ...
        jobagent.job_actor.load_state_dict(torch.load(f'{model_path}/job_actorckpt_{START_EPISODE}.pt'))
        jobagent.job_critic.load_state_dict(torch.load(f'{model_path}/job_critic_ckpt_{START_EPISODE}.pt'))

        # 2. 加载 AGV Agent
        agvagent.agv_actor.load_state_dict(torch.load(f'{model_path}/agv_actorckpt_{START_EPISODE}.pt'))
...
print(f"开始训练! Jobs:{JOB_RANGE}, Ops:{OPS_RANGE}, AGV:{TARGET_AGVS}")
