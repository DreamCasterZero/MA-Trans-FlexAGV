...
TARGET_MACHINES = 10      # 必须与 GNN 一致
TARGET_AGVS = 4          # 模拟硬件限制，固定为 2
JOB_RANGE = (12, 18)      # 【你的王牌】变长训练，体现泛化性
OPS_RANGE = (8, 12)       # 对齐 GNN 数据分布 (num_mas * 0.8 ~ 1.2)
TIME_RANGE = (1, 20)     # 对齐 GNN 数据分布

VAL_FOLDER_PATH = './validation_data/1510'
...
writer = SummaryWriter(log_dir=os.path.join(experiment_dir, "runs_gnn_15x10"))
...
if os.path.exists(VAL_FOLDER_PATH):
    ...
    print("请把 GNN 项目里的 data_dev/1510 文件夹复制到这里！")
    exit()
        ...
        if is_full_eval or avg_eval_score < best_avg_makespan:
            ...
            if is_full_eval:  # 只有全量评估才配谈“历史最佳”
                if avg_eval_score < best_avg_makespan:
                    ...
                    jobagent.save('best_15x10')
                    agvagent.save('best_15x10')
        # 定期保存 Checkpoint (防崩)
        if i % 5000 == 0:
            # 同时也保存常规 Checkpoint
