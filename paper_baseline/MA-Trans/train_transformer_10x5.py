...
TARGET_AGVS = 2          # 模拟硬件限制，固定为 2
JOB_RANGE = (6, 15)      # 【你的王牌】变长训练，体现泛化性
OPS_RANGE = (4, 6)       # 对齐 GNN 数据分布 (num_mas * 0.8 ~ 1.2)
TIME_RANGE = (1, 20)     # 对齐 GNN 数据分布

VAL_FOLDER_PATH = './validation_data/1005'

START_EPISODE = 0
TOTAL_EPISODES = 50000   # 小规模问题，5万轮通常足够收敛
...
writer = SummaryWriter(log_dir=os.path.join(experiment_dir, "runs_transformer_10x5"))
...
# 检查是否是 PyTorch 2.x 版本
if int(torch.__version__.split('.')[0]) >= 2:
    print("🔥 启用 torch.compile 加速 (Mode: reduce-overhead)...")
    try:
        # reduce-overhead 模式最适合 RL 这种小 Batch 循环
        jobagent.job_actor = torch.compile(jobagent.job_actor, mode='reduce-overhead')
        jobagent.job_critic = torch.compile(jobagent.job_critic, mode='reduce-overhead')
        agvagent.agv_actor = torch.compile(agvagent.agv_actor, mode='reduce-overhead')
        # 如果 agv_critic 也用到了 transformer，也可以加上
    except Exception as e:
        print(f"⚠️ 编译加速启动失败，回退到普通模式: {e}")

# 🔴🔴🔴【新增】断点续训代码 🔴🔴🔴
if START_EPISODE > 0:
    print(f"🔄 检测到断点续训，正在加载第 {START_EPISODE} 轮的模型权重...")

    # ⚠️ 请确保这里的路径和你文件夹里的文件名一致！
    # 通常在 TrainingResult/param/ 下面
    # 假设你的文件名格式是 job_actor_ckpt_15000.pt (你需要去文件夹确认一下具体名字)

    model_path = os.path.join(experiment_dir, 'param')

    # 1. 加载 Job Agent
    # 注意：这里需要加载 Actor 和 Critic
    # 如果你的 save() 函数保存了 actor 和 critic，你需要分别加载
    # 下面是假设的文件名，请务必去文件夹看一眼！
    try:
        # 这里使用了 PyTorch 标准加载方式
        # 假设文件名是 job_actor_ckpt_15000.pt (根据你的 save 函数逻辑)
        jobagent.job_actor.load_state_dict(torch.load(f'{model_path}/job_actor_ckpt_{START_EPISODE}.pt'))
        jobagent.job_critic.load_state_dict(torch.load(f'{model_path}/job_critic_ckpt_{START_EPISODE}.pt'))

        # 2. 加载 AGV Agent
        agvagent.agv_actor.load_state_dict(torch.load(f'{model_path}/agv_actor_ckpt_{START_EPISODE}.pt'))
        agvagent.agv_critic.load_state_dict(torch.load(f'{model_path}/agv_critic_ckpt_{START_EPISODE}.pt'))

        print("✅ 成功复活！模型已恢复到训练状态。")
        updates_already_done = START_EPISODE // BATCH_EPISODES

        # 2. 强制修改 Agent 内部的计数器
        # ⚠️ 注意：你需要去 gnn_ppo.py 确认一下那个计数变量叫什么名字！
        # 常见名字有：self.step, self.updates, self.t, self.count
        # 这里假设它叫 'step' (如果不报错就没问题，报错了就去看看 ppo 代码)
        ...
        print(f"❌ 加载失败！找不到文件：{e}")
        print("👉 请检查 START_EPISODE 是否正确，或者文件夹里有没有这个编号的文件。")
        exit()
...
for i in range(START_EPISODE, TOTAL_EPISODES + 1):
    ...
    if is_quick_eval or is_full_eval:
        ...
        if is_full_eval:
            ...
            # 随机抽 10 个来看看，省时间
            eval_candidates = random.sample(validation_set, min(10, len(validation_set)))
        ...
        if is_full_eval or avg_eval_score < best_avg_makespan:
            writer.add_scalar("Eval/GNN_Dataset_Score", avg_eval_score, i)
            # 保存历史最佳 (专门针对 10x5 优化)
            if is_full_eval:  # 只有全量评估才配谈“历史最佳”
                if avg_eval_score < best_avg_makespan:
                    best_avg_makespan = avg_eval_score
                    print(f"🏆 NEW BEST for 10x5! Saving model...")
                    jobagent.save('best_10x5')
                    agvagent.save('best_10x5')
        # 定期保存 Checkpoint (防崩)
        if i % 5000 == 0:
            # 同时也保存常规 Checkpoint
