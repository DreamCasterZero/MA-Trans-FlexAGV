...
TEST_DATA_DIR = './newtest/4010'

# 模型路径配置
MODEL_DIR = './result/20-10-5/param'  # 模型保存文件夹
CHECKPOINT_NAME = 'best_20x10'  # 检查点名称
...
def run_batch_test():
    for temp in temperatures:
        ...
        # 3. 加载权重 (只做一次)
        try:
            job_actor_path = os.path.join(MODEL_DIR, f'job_agent_{CHECKPOINT_NAME}.pt')
            agv_actor_path = os.path.join(MODEL_DIR, f'agv_agent_{CHECKPOINT_NAME}.pt')

            jobagent.load_snapshot(job_actor_path)
            agvagent.load_snapshot(agv_actor_path)
        ...
        with torch.no_grad():
            for i, file_path in enumerate(test_files):
                ...
                while not done:
                    # Job Action (Greedy)
                    j_act, _, _ = jobagent.choose_action(state, p_mask, j_mask, greedy=False, temperature=temp)

                    # Step 1
                    mid_s, p_mask, a_mask, m_act = env.job_step(j_act)

                    # AGV Action (Greedy)
                    a_act, _, _ = agvagent.choose_action(mid_s, p_mask, a_mask, greedy=False, temperature=temp)
