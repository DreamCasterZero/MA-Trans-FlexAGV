...
def run_batch_test():
    ...
    # 3. 加载权重 (只做一次)
    try:
        job_agent_path = os.path.join(MODEL_DIR, f'job_agent_{CHECKPOINT_NAME}.pt')
        agv_agent_path = os.path.join(MODEL_DIR, f'agv_agent_{CHECKPOINT_NAME}.pt')

        jobagent.load_snapshot(job_agent_path)
        agvagent.load_snapshot(agv_agent_path)
