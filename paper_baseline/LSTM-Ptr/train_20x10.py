...
writer = SummaryWriter(log_dir=os.path.join(experiment_dir, "runs_lstm_20x10"))
...
# ================= 🔄 新增：断点续训设置 =================
RESUME = True        # 🔴 开关：如果要继续训练，改为 True
if RESUME:
    # 定义断点路径 (根据你的命名规则)
        ...
        if is_full_eval or avg_eval_score < best_avg_makespan:
            ...
            if is_full_eval:  # 只有全量评估才配谈“历史最佳”
                if avg_eval_score < best_avg_makespan:
                    ...
                    print(f"🏆 NEW BEST for 20x10! Saving model...")
                    jobagent.save_snapshot(os.path.join(param_dir, 'job_agent_best_20x10.pt'))
                    agvagent.save_snapshot(os.path.join(param_dir, 'agv_agent_best_20x10.pt'))
        # 定期保存 Checkpoint (防崩)
        if i % 5000 == 0:
            # 同时也保存常规 Checkpoint
